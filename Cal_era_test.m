function [ coefficient_year_new ] = Cal_era_test( Xcoord,Ycoord,Zcoord )
%function [ COMPUTE_RBS ] = Cal_era_test( Xcoord,Ycoord,Zcoord )
%function [ Tm_Height ] = Cal_era_test( Xcoord,Ycoord,Zcoord )
...
Xcoord=55.00;Ycoord=110.00;Zcoord=0.0;
...
coefficient_new=Copy_of_lsq_heightcorrection_new(coefficient_yaoyibin,Pre_height_lsq);
Draw=Draw_picture(Pre_height_lsq,coefficient_yaoyibin,coefficient_new);
%coefficient_zhuhai=lsq_heightcorrection_zhuhai(Pre_height_lsq,coefficient_yaoyibin);


%Tm(count,1)  = cal_Tm( Ei,Ti,Hi,37 );
%for jj=1:1:size(Pre_height_lsq,1)
%    Tm_Height(count_h,1)  = Pre_height_lsq(jj,1);
%    Tm_Height(count_h,2)  = Pre_height_lsq(jj,2);
%    count_h=count_h+1;
%end

%COMPUTE_RBS  = COMPUTE_RMS_BIAS_STD( Pre_height_lsq,coefficient_cos ) ;

coefficient_year_new(count,:)=coefficient_yaoyibin';
count=count+1;
end
